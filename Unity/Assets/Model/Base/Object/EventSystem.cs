...
namespace ET
{
	public sealed class EventSystem: IDisposable
	{
        ...
        private readonly Dictionary<Type, IFacade> allFacades = new Dictionary<Type, IFacade>();
		...
		public void Add(Assembly assembly)
		{
            ...
            this.allFacades.Clear();
            foreach (Type type in types[typeof(FacadeAttribute)])
            {
                object obj = Activator.CreateInstance(type);
                IFacade iFacade = obj as IFacade;
                if (iFacade == null)
                {
                    Log.Error($"{obj.GetType().Name} 没有继承iFacade");
                }
                if (!this.allFacades.ContainsKey(type))
                {
                    this.allFacades.Add(type, iFacade);
                }
            }

            this.Load();
		}
        public Dictionary<Type, IFacade> GetFacades()
        {
            return allFacades;
        }
