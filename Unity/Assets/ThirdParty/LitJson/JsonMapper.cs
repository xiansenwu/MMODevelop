...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        private static void AddObjectMetadata (Type type)
        {
            ...
            foreach (PropertyInfo p_info in type.GetProperties ()) {
                JsonIgnoreAttribute jsonIgn = p_info.GetCustomAttribute<JsonIgnoreAttribute>();
                if (jsonIgn != null)
                {
                    continue;
                }
                ...
                JsonElementAttribute jsonEle = p_info.GetCustomAttribute<JsonElementAttribute>();
                if(jsonEle != null)
                {
                    p_data.propertyName = jsonEle.ElementName;
                }
                data.Properties.Add(p_data.propertyName, p_data);


            }

            foreach (FieldInfo f_info in type.GetFields ()) {
                JsonIgnoreAttribute jsonIgn = f_info.GetCustomAttribute<JsonIgnoreAttribute>();
                if (jsonIgn != null)
                {
                    continue;
                }
                ...
                JsonElementAttribute jsonEle = f_info.GetCustomAttribute<JsonElementAttribute>();
                if (jsonEle != null)
                {
                    p_data.propertyName = jsonEle.ElementName;
                }
...
//                 }
                                        ...
                                        int depth)
        {
            ...
            foreach (PropertyMetadata p_data in props) {
				...
				{
	                if (p_data.IsField) {
	                    writer.WritePropertyName (p_data.propertyName);
		...
		private static bool IsUnserializableUnityType(PropertyMetadata p_data)
		{
			...
			{
				PropertyInfo propertyInfo = (PropertyInfo)p_data.Info;
				type = propertyInfo.PropertyType;
			}
			return type == typeof(UnityEngine.GameObject) || type == typeof(UnityEngine.Transform) 
                || type == typeof(UnityEngine.Material) || type == typeof(UnityEngine.Object);
		}
